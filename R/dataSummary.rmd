...
dfClean2 %>% dplyr::group_by(PercentUnempPickup) %>% dplyr::summarize(percent_trips=round(100*n()/nrow(dfClean2),5),
  mean_fare = round(mean(fare,na.rm=TRUE),2) )%>% arrange(desc(percent_trips))
dfClean2 %>% dplyr::group_by(PercentUnempDropoff) %>% dplyr::summarize(percent_trips=round(100*n()/nrow(dfClean2),5),
  mean_fare = round(mean(fare,na.rm=TRUE),2) )%>% arrange(desc(percent_trips))
```
