...
colFilter <- c("company","payment_type","dropoff_area_group","tips","startDay","startMonth","startHour","pickup_area_group","fare","trip_total","trip_seconds","PercentDrivePickupF","PercentDriveDropoffF","MedIncomePickupF","MedIncomeDropoffF","Pop16PlusPickupF","Pop16PlusDropoffF","PercentUnempPickupF","PercentUnempDropoffF","hourlyRate","avgMPH","tipOutcome")

dfSlim <- dfClean2[colFilter] %>% drop_na()
...
phatV=list()
subsetTrain=1000000
subsetValidate=200000

phatT$logist = matrix(0.0,nrow(trainSample[1:subsetTrain,]),1)
phatV$logist = matrix(0.0,nrow(validateSample[1:subsetValidate,]),1)
#logistic regression, y=FALSE and model=FALSE to save memory
lgfit = glm(tipOutcome~.,trainSample[1:subsetTrain,],family="binomial",y=FALSE,model=TRUE)?pre
