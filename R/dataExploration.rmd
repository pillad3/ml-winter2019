...
df %>% group_by(dropoff_area) %>% tally(sort=TRUE)
#Trips by Pickup Area
df %>% group_by(pickup_area) %>% tally(sort=TRUE)

#Trips by Pickup Area
df %>% group_by(pickup_area) %>% summarize(total_fare = sum(fare,na.rm=TRUE),mean=mean(fare,na.rm=TRUE))
```
