...
Add column for date
```{r}
fareByMonth <- df %>% group_by(month=floor_date(df$startDate, "month")) %>%
   summarize(sum(fare))
ridesByMonth <- df %>% group_by(month=floor_date(df$startDate, "month")) %>%
   tally(sort=TRUE)
```


```{r}
plot(ridesByMonth$month,ridesByMonth$n)
```
```{r}
plot(fareByMonth$month,fareByMonth$totalFares)
```
